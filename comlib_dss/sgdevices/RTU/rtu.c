...
asduHandler(void* parameter, IMasterConnection connection, CS101_ASDU asdu)
{
    if (CS101_ASDU_getTypeID(asdu) == C_SC_NA_1) {
        ...
        if  (CS101_ASDU_getCOT(asdu) == CS101_COT_ACTIVATION) {
            ...
            int oa = InformationObject_getObjectAddress(io);
            if (oa >= 5000 && oa <= 5009) {
                SingleCommand sc = (SingleCommand) io;

                printf("IOA: %i switch to %i\n", oa, SingleCommand_getState(sc));
    ...
    if (CS101_ASDU_getTypeID(asdu) == C_DC_NA_1) {
        ...
        if (CS101_ASDU_getCOT(asdu) == CS101_COT_ACTIVATION) {
            ...
            if (oa >= 5000 && oa <= 5009) {
                ...
                int dcState = DoubleCommand_getState(dc);

                /* dcState values per IEC 60870-5-101 Table 64:
                 *   0 = not permitted
                 *   1 = OFF -> OPEN the breaker (trip / de-energise)
                 *   2 = ON  -> CLOSE the breaker (re-energise)
                 *   3 = not permitted
                 */
                const char* stateStr = (dcState == 1) ? "OPEN (trip)" :
                                       (dcState == 2) ? "CLOSE (reset)" : "INVALID";

                printf("IOA: %i double command -> %s (state=%i)\n",
                       oa, stateStr, dcState);
