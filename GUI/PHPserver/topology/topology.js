...
anychart.onDocumentReady(function () {
	...
	CB2.normal().labels().format("{%id}");
		...
		DSS1GW.hovered().labels().format("Denial of Service attack detected!");
                ...
                /* Restore breaker colour if tripped, otherwise clear stroke */
                if(CB_STATE1==1){ DSS1_RTU.normal().stroke("red",3); }
                else { DSS1_RTU.normal().stroke(null); }
                DDOS1=0;
		...
		DSS2GW.hovered().labels().format("Denial of Service attack detected!");
                ...
                /* Restore breaker colour if tripped, otherwise clear stroke */
                if(CB_STATE2==1){ DSS2_RTU.normal().stroke("red",3); }
                else { DSS2_RTU.normal().stroke(null); }
                DDOS2=0;
	...
	$.ajax({
            method: "POST",
            url: "../dbHandler/breakerState.php",
            dataType: "json"
        }).done(function(response){
            if(!response || response.length < 2){ return; }
            if(response[0]==1 && CB_STATE1!=1){
                /* DDoS alarm takes visual priority — only set red when no DDoS active */
                if(!DDOS1){ DSS1_RTU.normal().stroke("red",3); }
                DSS1_RTU.hovered().labels().format("IEC-104 command: BREAKER TRIPPED (OPEN)");
                CB_STATE1=1;
            }
            else if(response[0]==2 && CB_STATE1!=2){
                /* Only restore green when DDoS is not active */
                if(!DDOS1){ DSS1_RTU.normal().stroke("green",3); }
                DSS1_RTU.hovered().labels().format("{%id}");
                CB_STATE1=2;
            }
            if(response[1]==1 && CB_STATE2!=1){
                if(!DDOS2){ DSS2_RTU.normal().stroke("red",3); }
                DSS2_RTU.hovered().labels().format("IEC-104 command: BREAKER TRIPPED (OPEN)");
                CB_STATE2=1;
            }
            else if(response[1]==2 && CB_STATE2!=2){
                if(!DDOS2){ DSS2_RTU.normal().stroke("green",3); }
                DSS2_RTU.hovered().labels().format("{%id}");
                CB_STATE2=2;
            }
        })
